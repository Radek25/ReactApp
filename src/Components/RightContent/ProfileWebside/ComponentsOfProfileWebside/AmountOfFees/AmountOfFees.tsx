...
export const AmountOfFees: FC<ValueEdit> = (props) =>{
    ...
    return(
        ...
        <table>
            ...
            <tbody>
                <tr>
                    <td>{props.isOpen === false? yearOne : <select onChange={(e)=>setYearOne(e.target.value)}>{YearsArray.map(YearsArray => <option key={YearsArray}>{YearsArray}</option>)}</select>}</td>
                    <td>{props.isOpen === false? costOne : <input type='text' placeholder={costOne} onChange={(e)=>setCostOne(e.target.value)}/>}</td>
                    <td>{props.isOpen === false? (totalOne+currencyOne) : <div><input type='number' value={totalOne} onChange={(e)=>setTotalOne(parseFloat(e.target.value))}/><select onChange={(e)=>setCurrencyOne(e.target.value)}>{CurrencyArray.map(CurrencyArray => <option key={CurrencyArray}>{CurrencyArray}</option>)}</select></div>}</td>
                    <td>{props.isOpen === false? lawOne : <input type='text' placeholder={lawOne} onChange={(e)=>setLawOne(e.target.value)}/>}</td>
                </tr>
                <tr>
                    <td>{props.isOpen === false? yearTwo :  <select onChange={(e)=>setYearTwo(e.target.value)}>{YearsArray.map(YearsArray => <option key={YearsArray}>{YearsArray}</option>)}</select>}</td>
                    <td>{props.isOpen === false? costTwo: <input type='text' placeholder={costTwo} onChange={(e)=>setCostTwo(e.target.value)}/>}</td>
                    <td>{props.isOpen === false? (totalTwo+currencyTwo)  : <div><input type='number' value={totalTwo} onChange={(e)=>setTotalTwo(parseFloat(e.target.value))}/><select onChange={(e)=>setCurrencyTwo(e.target.value)}>{CurrencyArray.map(CurrencyArray => <option key={CurrencyArray}>{CurrencyArray}</option>)}</select></div>}</td>
